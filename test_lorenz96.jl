...
for model=1:length(models)
    model_errs = [0.5*diagm(0=>ones(D))]#Vector{Matrix{Float64}}(undef, 1)]
    biases = [zeros(D)]
...
biases = [zeros(D), zeros(D)]#[mean(infos[1].bias_hist[1000:end]), mean(infos[2].bias_hist[1000:end])]
#[zeros(D), zeros(D)]
ensembles = [x0 .+ rand(MvNormal(R), ens_sizes[model]) for model=1:n_models]
ρ = 1e-3
model_errs = [0.5*diagm(0=>ones(D)), 0.5*diagm(0=>ones(D))]#[mean(infos[1].Q_hist[1000:end]),
